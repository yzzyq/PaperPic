...
##    fig,ax = plt.subplots()
    ...
    x_data = ['ASUWO', 'BSVM', 'MSSVM', 'multitrain', 'ssoSMOTEsso','VBensembles','wdchange']
...
##    fig,ax = plt.subplots()
    ...
    rects1 = ax.bar(x - width, three_precise[:,0], width, label='3611', zorder=1,facecolor = color_set[0],linewidth=1, linestyle = '--', edgecolor = 'black')
    rects2 = ax.bar(x, three_precise[:,1], width, label='8202', zorder=1, facecolor = color_set[1], edgecolor = 'white')
    rects3 = ax.bar(x + width, three_precise[:,2], width, label='19411', zorder=1, facecolor = color_set[2],linewidth=1, linestyle = '-', edgecolor = 'black')
    ...
    # plt.xticks(x_data)
    ax.set_xticklabels(('0','ASUWO', 'BSVM', 'MSSVM', 'multitrain', 'ssoSMOTEsso','VBensembles','本方法'),fontsize=13)
    ...
    # plt.ylim(-1.2,1.2)
    plt.xlabel('对比的算法以及本方法',fontsize=13)
    plt.ylabel('时间（S）',fontsize=13)
    
    labels = ['数据量:','3611', '8202', '19411']
    linestyles = ['-','--','-','-']
    edgecolors = ['none','black','none','black']
    color_set = ['none','white','black','white']
    ...
    ax.legend(handles=patches, bbox_to_anchor=(0.8, 1.07), ncol=4, edgecolor='white', handlelength=0.8, fontsize=12)
